/* letterboxd-overlay.js — плавающая кнопка + фуллскрин оверлей, ES5
   Бьём ровно по ?user&pages, показываем тело 500-ошибки.
*/
(function () {
  'use strict';
  ...
  function ensureStyles() {
    ...
    var css =
      '#lb-ov{position:fixed;inset:0;z-index:99999;background:rgba(12,12,12,.96);display:flex;flex-direction:column;color:#fff}' +
  ...
  function openOverlay() {
    ensureStyles();
    if (document.getElementById('lb-ov')) return;
    ...
    function close(){ try{ document.body.removeChild(ov); }catch(_){} window.removeEventListener('keydown', onKey); }
    function onKey(e){ var c=e.keyCode||e.which; if (c===27||c===8||c===461||c===10009) close(); }
    ...
    function load() {
      ...
      status.style.display = ''; grid.style.display = 'none'; status.textContent = 'Загрузка…';

      var url = WORKER_URL + '/?user=' + encodeURIComponent(LETTERBOXD_USER) +
                '&pages=' + encodeURIComponent(PAGES);  // только user & pages
      ...
      httpGet(url, function (text) {
        ...
        for (var i=0;i<items.length;i++){
          var it = items[i];
          var card = document.createElement('div'); card.className='card selector focusable'; card.setAttribute('tabindex','0');
          var poster = document.createElement('div'); poster.className='poster'; if (it.poster) poster.style.backgroundImage='url('+it.poster+')';
          var cap = document.createElement('div'); cap.className='caption'; cap.textContent=(it.title||'Без названия')+(it.year?' ('+it.year+')':'');
          card.appendChild(poster); card.appendChild(cap);

          (function(item){
            function openFull(){
              try { Lampa.Activity.push({ title:item.title, url:'', component:'full', id:item.tmdb_id, method:'movie' }); }
              catch(e){ try{ window.open('https://www.themoviedb.org/movie/'+item.tmdb_id, '_blank'); }catch(_){ noty('Не удалось открыть карточку'); } }
            }
        ...
        status.style.display = 'none'; grid.style.display = '';
      }, function (err) {
        var body = ''; try { body = JSON.parse(err.body).error; } catch(_) { body = err.body; }
        ov.querySelector('.status').innerHTML = 'Сеть/доступ: HTTP ' + (err.status||'') + (body ? '<br>'+String(body) : '');
      });
    }
  }
  ...
  function injectButton() {
    ...
    btn.style.cssText = 'position:fixed;top:12px;right:12px;z-index:99999;padding:8px 12px;background:#2b2b2b;color:#fff;border-radius:8px;font-size:14px;opacity:.9;cursor:pointer';
